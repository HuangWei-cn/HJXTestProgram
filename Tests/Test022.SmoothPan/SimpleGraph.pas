...
begin
  ...
  begin
    ...
    begin
      ...
      //2018-09-25 下面的修改及SimpleGraph.MouseMove的修改可帶來平滑的PAN模式體驗
      if Kind = sbHorizontal then
      begin
        if owner.CommandMode <> cmpan then //2018-09-25
        Owner.ScrollBy(OldPos - Value, 0)  //origin
      end
      else
        if owner.commandmode <> cmpan  then  //2018-09-25
        Owner.ScrollBy(0, OldPos - Value);   //origin
...
begin
  ...
  begin
    ...
    begin
      //2018-09-25 增加下面一行代碼，配合TGraphScrollBar的SetPostiion方法的修改，可帶來平滑
      //的PAN模式體驗
      Scrollby(x-fDragSourcePt.x,y-fdragsourcept.y);

      with HorzScrollBar do
        if IsScrollBarVisible then
        begin
          NewPos := Position + (fDragSourcePt.X - X);
          npx := fdragsourcept.x-x;
