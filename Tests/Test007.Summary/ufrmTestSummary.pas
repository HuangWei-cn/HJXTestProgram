...
begin
  ...
  //IAppServices.OpenDatabaseManager; //由于修改了OpenProject的参数，导致暂时从接口无法调用 2022-11-3

  //因修改了OpenProject的参数，现在暂时只能直接调用。参数True指打开一个新的配置文件
...
begin
  ...
  lblPosition.Caption := '';
  //2022-11-3 启动即自动加载配置文件，参数LoadNew=False表示不加载新参数，从ini文件中找到配置文件的
  //位置和名称，然后加载
  Timer1.Enabled := True; //没法子，得先显示界面再自动加载参数，300ms后自动加载，反正也不慢。
...
{-----------------------------------------------------------------------------
  ...
  Description: 之所以要用一个Timer来调用加载工程文件，是因为希望先显示程序界面
  然后再加载。
-----------------------------------------------------------------------------}
